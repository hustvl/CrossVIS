...
class Conv2D(nn.Module):
    def __init__(self,
        ...
        assert type(kernel_size) in [
            int, tuple
        ], 'Allowed kernel type [int or tuple], not {}'.format(
            type(kernel_size))
    ...
    def forward(self, x):
        ...
        if self.padding == 'same':
            ...
            h_pad_need = max(0, (height - 1) * self.stride + self.h_kernel -
                             height)
            w_pad_need = max(0,
                             (width - 1) * self.stride + self.w_kernel - width)
...
class GCN(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, k=3):
        ...
        self.conv_l1 = Conv2D(in_channels=in_channels,
                              out_channels=out_channels,
                              kernel_size=(k, 1),
                              padding='same')
        self.conv_l2 = Conv2D(in_channels=out_channels,
                              out_channels=out_channels,
                              kernel_size=(1, k),
                              padding='same')

        self.conv_r1 = Conv2D(in_channels=in_channels,
                              out_channels=out_channels,
                              kernel_size=(1, k),
                              padding='same')
        self.conv_r2 = Conv2D(in_channels=out_channels,
                              out_channels=out_channels,
                              kernel_size=(k, 1),
                              padding='same')
