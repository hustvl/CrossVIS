...
class Tree(nn.Module):
    def __init__(self,
        ...
        if levels == 1:
            self.tree1 = block(cfg,
                               in_channels,
                               out_channels,
                               stride,
                               dilation=dilation)
            self.tree2 = block(cfg,
                               out_channels,
                               out_channels,
                               1,
                               dilation=dilation)
...
def dla34(cfg, pretrained=None, **kwargs):  # DLA-34
    model = DLA(cfg, [1, 1, 1, 2, 2, 1], [16, 32, 64, 128, 256, 512],
                block=BasicBlock,
                **kwargs)
...
def dla46_c(cfg, pretrained=None, **kwargs):  # DLA-46-C
    ...
    model = DLA(cfg, [1, 1, 1, 2, 2, 1], [16, 32, 64, 64, 128, 256],
                block=Bottleneck,
                **kwargs)
    if pretrained is not None:
...
def dla60x_c(cfg, pretrained=None, **kwargs):  # DLA-X-60-C
    BottleneckX.expansion = 2
    model = DLA(cfg, [1, 1, 1, 2, 3, 1], [16, 32, 64, 64, 128, 256],
                block=BottleneckX,
                **kwargs)
    if pretrained is not None:
...
def dla60(cfg, pretrained=None, **kwargs):  # DLA-60
    Bottleneck.expansion = 2
    model = DLA(cfg, [1, 1, 1, 2, 3, 1], [16, 32, 128, 256, 512, 1024],
                block=Bottleneck,
                **kwargs)
    if pretrained is not None:
