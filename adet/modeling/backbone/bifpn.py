...
class FeatureMapResampler(nn.Module):
    def __init__(self, in_channels, out_channels, stride, norm=''):
        ...
        if in_channels != out_channels:
            self.reduction = Conv2d(in_channels,
                                    out_channels,
                                    kernel_size=1,
                                    bias=(norm == ''),
...
class BackboneWithTopLevels(Backbone):
    def __init__(self, backbone, out_channels, num_top_levels, norm=''):
        ...
        last_feature_name = max(self._out_feature_strides.keys(),
                                key=lambda x: split_name(x)[1])
            ...
            for input_offset in inputs_offsets:
                ...
                if h > target_h and w > target_w:
                    ...
                    if h < target_h or w < target_w:
                        input_node = F.interpolate(input_node,
...
class BiFPN(Backbone):
    ...
    def __init__(self,
        ...
        self._size_divisibility = bottom_up_output_shapes[
            in_features[-1]].stride
        self.out_channels = out_channels
