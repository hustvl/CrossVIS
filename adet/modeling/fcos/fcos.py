...
class FCOS(nn.Module):
    ...
    def forward_head(self, features, top_module=None):
        ...
        pred_class_logits, pred_deltas, pred_centerness, top_feats, bbox_towers, reid_feats = self.fcos_head(
            features, top_module, self.yield_proposal)
        return pred_class_logits, pred_deltas, pred_centerness, top_feats, bbox_towers, reid_feats
    ...
    def forward(self, images, features, gt_instances=None, top_module=None):
        ...
        logits_pred, reg_pred, ctrness_pred, top_feats, bbox_towers, reid_feats = self.fcos_head(
            features, top_module, self.yield_proposal or self.yield_box_feats)

        if self.training:
            results, losses = self.fcos_outputs.losses(logits_pred, reg_pred,
            ...
            if self.yield_proposal:
                with torch.no_grad():
                    results['proposals'] = self.fcos_outputs.predict_proposals(
                        logits_pred, reg_pred, ctrness_pred, locations,
                        images.image_sizes, top_feats)
            if self.yield_box_feats:
                results['box_feats'] = {
                    f: b
                    for f, b in zip(self.in_features, bbox_towers)
                }
            return results, losses
        else:
            results = self.fcos_outputs.predict_proposals(
                logits_pred, reg_pred, ctrness_pred, locations,
                images.image_sizes, top_feats, reid_feats)
            extras = {}
            if self.yield_box_feats:
                extras['box_feats'] = {
                    f: b
...
class FCOSHead(nn.Module):
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        self.crossvis = cfg.MODEL.CROSSVIS_ON
        ...
        self.cls_logits = nn.Conv2d(in_channels,
                                    ...
                                    padding=1)
        if self.crossvis:
            self.reid_pred = nn.Conv2d(in_channels,
                                       256,
                                       kernel_size=1,
                                       stride=1,
                                       padding=0)
            self.reid_pred_bn = nn.BatchNorm2d(256)
        ...
        for modules in [self.reid_tower, self.reid_pred]:
            for l in modules.modules():
                if isinstance(l, nn.Conv2d):
                    torch.nn.init.normal_(l.weight, std=0.01)
                    torch.nn.init.constant_(l.bias, 0)
    ...
    def forward(self, x, top_module=None, yield_bbox_towers=False):
        ...
        for l, feature in enumerate(x):
            ...
            if self.crossvis:
                reid_tower = self.reid_tower(feature)
            ...
            if self.crossvis:
                reid_feats.append(self.reid_pred_bn(
                    self.reid_pred(reid_tower)))
        ...
        return logits, bbox_reg, ctrness, top_feats, bbox_towers, reid_feats
