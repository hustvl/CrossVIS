...
from .utils import center_of_mass, imrescale, mask_nms, matrix_nms, point_nms
...
class SOLOv2(nn.Module):
    ...
    Creates FPN backbone, instance branch for kernels and categories
    prediction, mask branch for unified mask features. Calculates and applies
    proper losses to class and masks.
    """
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            results = self.inference(cate_pred, kernel_pred, mask_pred,
                                     images.image_sizes, batched_inputs)
            return results
    ...
    def inference(self, pred_cates, pred_kernels, pred_masks, cur_sizes,
                  images):
        ...
        for img_idx in range(len(images)):
            ...
            pred_kernel = [
                pred_kernels[i][img_idx].permute(1, 2, 0).view(
                    -1, self.num_kernels).detach()
                for i in range(num_ins_levels)
            ]
            ...
            result = self.inference_single_image(pred_cate, pred_kernel,
                                                 pred_mask, cur_sizes[img_idx],
                                                 ori_size)
...
class SOLOv2InsHead(nn.Module):
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        head_configs = {
            'cate': (cfg.MODEL.SOLOV2.NUM_INSTANCE_CONVS,
        ...
        self.cate_pred = nn.Conv2d(self.instance_channels,
                                   self.num_classes,
                                   kernel_size=3,
                                   stride=1,
                                   padding=1)
        self.kernel_pred = nn.Conv2d(self.instance_channels,
                                     self.num_kernels,
                                     kernel_size=3,
                                     stride=1,
                                     padding=1)
    ...
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        for i in range(self.num_levels):
            ...
            if i == 0:
                ...
                convs_per_level.add_module('conv' + str(i),
            ...
            for j in range(i):
                if j == 0:
                    ...
                    convs_per_level.add_module('conv' + str(j),
                                               nn.Sequential(*conv_tower))
                    upsample_tower = nn.Upsample(scale_factor=2,
                                                 mode='bilinear',
                                                 align_corners=False)
        ...
        self.conv_pred = nn.Sequential(
            nn.Conv2d(self.mask_channels,
                      ...
                      stride=1,
                      padding=0,
                      bias=norm is None), nn.GroupNorm(32, self.num_masks),
