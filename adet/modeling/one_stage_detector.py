...
from detectron2.modeling.postprocessing import \
    detector_postprocess as d2_postprocesss
...
def detector_postprocess(results,
    ...
    results = d2_postprocesss(results, output_height, output_width,
                              mask_threshold)
...
class OneStageRCNN(GeneralizedRCNN):
    ...
    def inference(self,
        ...
        if do_postprocess:
            return OneStageRCNN._postprocess(results, batched_inputs,
                                             images.image_sizes)
        else:
            return results
