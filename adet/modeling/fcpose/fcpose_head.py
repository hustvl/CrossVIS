...
class fcpose_head_module(nn.Module):
    def __init__(self, cfg):
        ...
        self.sizes_of_interest = nn.Parameter(
            torch.tensor(cfg.MODEL.FCOS.SIZES_OF_INTEREST +
    ...
    def forward(self,
                ...
                base_stride=8):
        bases = bases[0]
        bases, direction = torch.split_with_sizes(bases, [self.mask_dim, 34],
                                                  dim=1)
        base_locations = compute_locations_per_level(bases.size(2),
                                                     bases.size(3),
                                                     stride=base_stride,
                                                     device=bases.device)
        ...
        direction_locations = direction_locations.permute(
            2, 0, 1)[None].repeat(N, 17, 1, 1)  # N, 17*2, H ,W
        ...
        if self.training:
            ...
            offsets = locations.reshape(-1, 1, 2) - base_locations.reshape(
                1, -1, 2)
            offsets = offsets.permute(0, 2, 1).float() / max_ranges.reshape(
                -1, 1, 1).float()
            offsets = torch.cat(
                [offsets, bases[im_inds].reshape(n_inst, num_bases, -1)],
                dim=1)
            offsets = offsets.reshape(1, -1, H, W)

            assert not torch.isnan(attns).any()
            attns = self.atten_producer(attns)
            weights, biases = get_subnetworks_params(attns, num_bases,
            ...
            mask_logits = subnetworks_forward(offsets, weights, biases,
                                              n_inst).squeeze()
            ...
            mask_logits = mask_logits.flatten(start_dim=2).softmax(
                dim=2).reshape(-1, 17, H, W)
            direction = direction[:, :, :, :,
                                  None].permute(0, 2, 3, 4, 1).reshape(
                                      n_inst, H, W, 17, 2)
            ...
            if total_instances == 0:
                ...
                for box in proposals:
                    box.pred_keypoints = box.pred_classes.new_full((0, 17, 3),
                                                                   0).float()
                return proposals, {}
            ...
            for im_i in range(len(proposals)):
                ...
                max_ranges = self.sizes_of_interest[
                    proposals[im_i].fpn_levels.long()]

                offsets = per_locations.reshape(
                    -1, 1, 2) - base_locations.reshape(1, -1, 2)
                offsets = offsets.permute(
                    0, 2, 1).float() / max_ranges.reshape(-1, 1, 1).float()
                # offsets = offsets.tanh()
                offsets = torch.cat([
                    offsets, bases[im_i].reshape(1, num_bases, -1).expand(
                        n_inst, -1, -1)
                ],
                                    dim=1)
                offsets = offsets.reshape(1, -1, H, W)
                ...
                pred_mask_logits = subnetworks_forward(offsets, weights,
                                                       biases, n_inst)
                ...
                direction = direction[:, :, :, :,
                                      None].permute(0, 2, 3, 4, 1).reshape(
                                          n_inst, H, W, 17, 2)
                pred_mask_logits = pred_mask_logits.permute(0, 2, 3,
                                                            1)[:, :, :, :,
                                                               None]

                pred_mask_logits = pred_mask_logits.reshape(
                    n_inst, H * W, 17).permute(0, 2, 1)  #.sigmoid()
                ...
                vis = max_value.reshape(pred_keypoints.size(0),
                                        pred_keypoints.size(1), 1)
