...
from .utils import compute_basis_stride, process_gt_instances, top_module
...
class FCPose(nn.Module):
    ...
    def forward(self, images, features, gt_instances=None):
        ...
        proposals, proposal_losses = self.fcos(images, features, gt_instances,
                                               self.top_module)

        basis_out, basis_losses = self.basis_module(features, basis_gt_heatmap,
                                                    p3_heatmap_list)
        ...
        detector_results, detector_losses = self.fcpose_head(
            basis_out['bases'], proposals, head_gt_heatmap, gt_instances,
            basis_out['basis_seg'])
