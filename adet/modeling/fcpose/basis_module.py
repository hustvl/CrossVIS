...
import torch
from detectron2.layers import ConvTranspose2d, ShapeSpec
...
class basis_module(nn.Module):
    def __init__(self, cfg, input_shape: Dict[str, ShapeSpec]):
        ...
        if self.loss_on:
            ...
            self.p3_logits = nn.Conv2d(planes,
                                       self.num_classes,
                                       kernel_size=1,
                                       stride=1)
            self.upsampler = nn.Sequential(
                ConvTranspose2d(planes + self.num_classes,
                                planes,
                                8,
                                stride=4,
                                padding=6 // 2 - 1),
            ...
            self.p1_logits = nn.Conv2d(planes,
                                       self.num_classes,
                                       kernel_size=3,
                                       stride=1,
                                       padding=1)
    ...
    def forward(self, features, p1_heatmap_list=None, p3_heatmap_list=None):
        ...
        if self.training and self.loss_on:
            ...
            p1_loss, p3_loss = compute_loss(p1_heatmap_list, p3_heatmap_list,
                                            p1_logits, p3_logits)
