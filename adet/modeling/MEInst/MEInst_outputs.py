...
class MEInstOutputs(object):
    ...
    @torch.no_grad()
    def prepare_masks(self, proposals: List[Instances],
    ...
    def _get_ground_truth(self):
        ...
        mask_targets_split = []
        for mask_target_per_img, mask_index_per_img in zip(
                mask_targets, mask_indices):
            ...
            for level_e in num_loc_list:
                ...
                mask_target_per_level = mask_target_per_img[
                    index_level].gt_masks.crop_and_resize(
        ...
        for level in range(len(self.locations)):
            mask_targets_level_first.append(
                torch.cat(
                    [mask_per_im[level] for mask_per_im in mask_targets_split],
                    dim=0))
    ...
    def losses(self):
        """Return the losses from a set of MEInst predictions and their
        ...
        return self.MEInst_losses(labels, reg_targets, logits_pred, reg_pred,
                                  ctrness_pred, mask_pred, mask_targets)

    def predict_proposals(self):
        sampled_boxes = []

        bundle = (self.locations, self.logits_pred, self.reg_pred,
                  self.ctrness_pred, self.strides, self.mask_regression)
        ...
        for i, (l, o, r, c, s, mr) in enumerate(zip(*bundle)):
            ...
            sampled_boxes.append(
                self.forward_for_single_feature_map(l, o, r, c, mr,
                                                    self.image_sizes))
        ...
        for i in range(num_images):
            ...
            per_image_masks = self.mask_encoding.decoder(per_image_masks,
                                                         is_train=False)
            per_image_masks = per_image_masks.view(-1, 1, self.mask_size,
                                                   self.mask_size)
    ...
    def forward_for_single_feature_map(self, locations, box_cls, reg_pred,
        ...
        mask_regression = mask_regression.view(N, self.dim_mask, H,
                                               W).permute(0, 2, 3, 1)
