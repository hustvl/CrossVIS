...
class PCAMaskEncoding(nn.Module):
    ...
    def __init__(self, cfg):
        ...
        if self.agnostic:
            self.components = nn.Parameter(torch.zeros(self.dim_mask,
                                                       self.mask_size**2),
                                           requires_grad=False)
            self.explained_variances = nn.Parameter(torch.zeros(self.dim_mask),
                                                    requires_grad=False)
            self.means = nn.Parameter(torch.zeros(self.mask_size**2),
                                      requires_grad=False)
        else:
    ...
    def inverse_sigmoid(self, x):
        ...
        value_random = torch.where(value_random > VALUE_MIN, value_random,
                                   VALUE_MIN * torch.ones_like(x))
    ...
    def encoder(self, X):
        ...
        assert X.shape[1] == self.mask_size**2, print(
            'The original mask_size of input'
