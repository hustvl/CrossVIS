...
from adet.layers import GCN, DFConv2d, IOULoss, NaiveGroupNorm
...
class MEInst(nn.Module):
    ...
    def forward(self, images, features, gt_instances):
        ...
        logits_pred, reg_pred, ctrness_pred, bbox_towers, mask_regression = self.MEInst_head(
            features)
        ...
        if self.training:
            ...
            if not self.flag_parameters:
                ...
                components_path = components_path.replace(
                    'agnosticTrue',
                    'agnostic' + str(self.cfg.MODEL.MEInst.AGNOSTIC))
                components_path = components_path.replace(
                    'whitenTrue', 'whiten' + str(self.cfg.MODEL.MEInst.WHITEN))
                components_path = components_path.replace(
                    'sigmoidTrue',
                    'sigmoid' + str(self.cfg.MODEL.MEInst.SIGMOID))
                components_path = components_path.replace(
                ...
                with torch.no_grad():
                    if self.cfg.MODEL.MEInst.AGNOSTIC:
                        components = nn.Parameter(torch.from_numpy(
                            parameters['components_c'][0]).float().to(device),
                                                  requires_grad=False)
                        explained_variances = nn.Parameter(torch.from_numpy(
                            parameters['explained_variance_c'][0]).float().to(
                                device),
                                                           requires_grad=False)
                        means = nn.Parameter(torch.from_numpy(
                            parameters['mean_c'][0]).float().to(device),
                                             requires_grad=False)
...
class MEInstHead(nn.Module):
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        head_configs = {
            'cls':
            (cfg.MODEL.MEInst.NUM_CLS_CONVS, cfg.MODEL.MEInst.USE_DEFORMABLE),
            'bbox':
            (cfg.MODEL.MEInst.NUM_BOX_CONVS, cfg.MODEL.MEInst.USE_DEFORMABLE),
            'share': (cfg.MODEL.MEInst.NUM_SHARE_CONVS,
                      cfg.MODEL.MEInst.USE_DEFORMABLE),
            'mask':
            (cfg.MODEL.MEInst.NUM_MASK_CONVS, cfg.MODEL.MEInst.USE_DEFORMABLE)
        }
