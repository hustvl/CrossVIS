...
def parse_args():
    parser = argparse.ArgumentParser(
        description='PCA Mask Encoding for local mask.')
...
if __name__ == '__main__':
    ...
    # build data loader.
    mask_data = MaskLoader(root=dataset_root,
                           dataset=args.dataset,
                           size=mask_size)
    mask_loader = DataLoader(mask_data,
    ...
    components_c = np.concatenate(components_c).mean(0)[
    ...
    explained_variance_c = np.concatenate(explained_variance_c).mean(0)[
        np.newaxis, :].astype(np.float32)
    print('The mean variance_ratio for all categories is {}'.format(
        np.mean(ratio)))

    # save the parameters.
    output_path = os.path.join(
        output_dir, args.dataset + '_class_agnostic' + str(class_agnostic) +
        '_whiten' + str(whiten) + '_sigmoid' + str(sigmoid) + '_' +
        str(n_components) + '.npz')
