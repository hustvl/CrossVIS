...
class Blender(object):
    def __init__(self, cfg):
        ...
        self.pooler = ROIPooler(output_size=self.pooler_resolution,
    ...
    def __call__(self, bases, proposals, gt_instances):
        if gt_instances is not None:
            ...
            if pos_inds.numel() == 0:
                return None, {
                    'loss_mask':
                    sum([x.sum() * 0 for x in attns]) + bases[0].sum() * 0
                }
            ...
            for instances_per_image in gt_instances:
                ...
                gt_mask_per_image = instances_per_image.gt_masks.crop_and_resize(
                    instances_per_image.gt_boxes.tensor,
                    self.pooler_resolution).to(device=pred_mask_logits.device)
            ...
            pred_mask_logits = pred_mask_logits.view(-1, 1,
