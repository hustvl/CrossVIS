...
class BAText(nn.Module):
    ...
    def forward(self, images, features, gt_instances=None, top_module=None):
        ...
        outputs = BATextOutputs(images, locations, logits_pred, reg_pred,
                                ctrness_pred, top_feats, self.focal_loss_alpha,
                                self.focal_loss_gamma, self.iou_loss,
                                self.center_sample, self.sizes_of_interest,
                                self.strides, self.radius,
                                self.fcos_head.num_classes, pre_nms_thresh,
                                pre_nms_topk, self.nms_thresh, post_nms_topk,
                                self.thresh_with_ctr, gt_instances)
...
class FCOSHead(nn.Module):
    def __init__(self, cfg, input_shape: List[ShapeSpec]):
        ...
        head_configs = {
            ...
            'share':
            (cfg.MODEL.FCOS.NUM_SHARE_CONVS, cfg.MODEL.FCOS.USE_DEFORMABLE)
        }
