...
def default_evaluation_params():
    ...
    return {
        ...
        'CONFIDENCES':
        False,  #Detections must include confidence value. MAP and MAR will be calculated,
...
def validate_data(gtFilePath, submFilePath, evaluationParams):
    ...
    Validates also that there are no missing files in the folder. If some error
    detected, the method raises the error
    """
...
def evaluate_method(gtFilePath, submFilePath, evaluationParams):
    """Method evaluate_method: evaluate method and returns the results Results.
    ...
    def polygon_from_points(points):
        """Returns a Polygon object to use with the Polygon2 class from a list
        of 8 points: x1,y1,x2,y2,x3,y3,x4,y4."""
    ...
    def include_in_dictionary(transcription):
        """Function used in Word Spotting that finds if the Ground Truth
        transcription meets the rules to enter into the dictionary.
    ...
    def include_in_dictionary_transcription(transcription):
        """Function applied to the Ground Truth transcriptions used in Word
        Spotting.
...
def text_eval_main(det_file, gt_file, is_word_spotting):
    ...
    return rrc_evaluation_funcs.main_evaluation(None, det_file, gt_file,
