...
def load_zip_file(file, fileNameRegExp='', allEntries=False):
    """Returns an array with the contents (filtered by fileNameRegExp) of a ZIP
    file.
...
def validate_lines_in_file_gt(fileName,
                              file_contents,
                              CRLF=True,
                              LTRB=True,
    ...
    for line in lines:
        ...
        if (line != ''):
            try:
                validate_tl_line_gt(line, LTRB, withTranscription,
...
def validate_tl_line_gt(line,
    ...
    """
    get_tl_line_values_gt(line, LTRB, withTranscription, withConfidence,
                          imWidth, imHeight)
...
def get_tl_line_values_gt(line,
                          LTRB=True,
                          withTranscription=False,
                          withConfidence=False,
                          imWidth=0,
    ...
    if LTRB:
        ...
        if (imWidth > 0 and imHeight > 0):
            for ip in range(0, len(points), 2):
                validate_point_inside_bounds(points[ip], points[ip + 1],
                                             imWidth, imHeight)
...
def validate_clockwise_points(points):
    """Validates that the points that the 4 points that dlimite a polygon are
    in clockwise order."""
...
def get_tl_line_values_from_file_contents_det(content,
                                              CRLF=True,
                                              LTRB=True,
                                              withTranscription=False,
                                              withConfidence=False,
...
def main_validation(default_evaluation_params_fn, validate_data_fn):
    ...
    try:
        ...
        evalParams = default_evaluation_params_fn()
        if 'p' in p.keys():
            evalParams.update(p['p'] if isinstance(p['p'], dict) else json.
                              loads(p['p'][1:-1]))
