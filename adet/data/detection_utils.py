...
def transform_instance_annotations(annotation,
    ...
    if 'beziers' in annotation:
        beziers = transform_beziers_annotations(annotation['beziers'],
                                                transforms)
...
def annotations_to_instances(annos, image_size, mask_format='polygon'):
    ...
    if 'id' in annos[0]:
        ids = [obj.get('id', []) for obj in annos]
        instance.id = torch.as_tensor(ids, dtype=torch.int32)
...
class HeatmapGenerator():
    def __init__(self, num_joints, sigma, head_sigma):
        ...
        y = x[:, np.newaxis]
        x0, y0 = (size - 1) / 2, (size - 1) / 2
        self.p3_g = np.exp(-((x - x0)**2 + (y - y0)**2) /
                           (2 * self.p3_sigma**2))
        ...
        y = x[:, np.newaxis]
        x0, y0 = (size - 1) / 2, (size - 1) / 2
        self.head_g = np.exp(-((x - x0)**2 + (y - y0)**2) /
                             (2 * head_sigma**2))
        ...
        for p in joints:
            ...
            for idx, pt in enumerate(p):
                if pt[2] > 0:
                    ...
                    aa, bb = max(0, ul[1]), min(br[1], h)
                    head_hms[idx, aa:bb,
                             cc:dd] = np.maximum(head_hms[idx, aa:bb, cc:dd],
                                                 self.head_g[a:b, c:d])
        ...
        for p in joints:
            ...
            for idx, pt in enumerate(p):
                if pt[2] > 0:
                    ...
                    aa, bb = max(0, ul[1]), min(br[1], h)
                    p3_hms[idx, aa:bb,
                           cc:dd] = np.maximum(p3_hms[idx, aa:bb, cc:dd],
                                               self.p3_g[a:b, c:d])
