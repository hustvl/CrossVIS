...
class Beziers:
    """This structure stores a list of bezier curves as a Nx16 torch.Tensor. It
    will support some common methods about bezier shapes (`area`, `clip`,
    `nonempty`, etc), and also behaves like a Tensor (support indexing,
    `to(device)`, `.device`, and iteration over all boxes)
    ...
    def __init__(self, tensor: torch.Tensor):
        ...
        if tensor.numel() == 0:
            ...
            tensor = tensor.reshape((0, 16)).to(dtype=torch.float32,
                                                device=device)
