"""A working example to export the R-50 based FCOS model: python
...
from detectron2.modeling import ProposalNetwork, build_model
from detectron2.utils.logger import setup_logger
from torch import nn
from torch.nn import functional as F

from adet.config import get_cfg
from adet.modeling import FCOS, BAText, BlendMask, MEInst, SOLOv2, condinst
...
def patch_fcos(cfg, proposal_generator):
    def proposal_generator_forward(self,
                                   images,
                                   features,
                                   gt_instances=None,
                                   top_module=None):
        ...
        return (logits_pred, reg_pred, ctrness_pred, top_feats,
                bbox_towers), None

    proposal_generator.forward = types.MethodType(proposal_generator_forward,
                                                  proposal_generator)
...
def upsample(
        tensor,
        factor):  # aligned_bilinear in adet/utils/comm.py is not onnx-friendly
    assert tensor.dim() == 4
    assert factor >= 1
...
def main():
    ...
    if hasattr(model, 'mask_branch'):
        if isinstance(model.mask_branch, MaskBranch):
            patch_mask_branch(
                cfg, model.mask_branch
            )  # replace aligned_bilinear with nearest upsample
