...
error_list = [
    '23382.png', '23441.png', '20714.png', '20727.png', '23300.png',
    '21200.png'
]
...
def gen_coco(phase):
    ...
    for index, image_name in enumerate(files):
        ...
        if image_name in error_list:
            continue
        instance = cv2.imread(os.path.join('instance', phase, image_name),
                              flags=cv2.IMREAD_GRAYSCALE)
        semantic = cv2.imread(os.path.join('semantic', phase, image_name),
                              flags=cv2.IMREAD_GRAYSCALE)
        ...
        for instance_id in instance_ids:
            ...
            if store_segmentation:
                contours, hierarchy = cv2.findContours(
                    (instance_part * 255).astype(np.uint8), cv2.RETR_TREE,
